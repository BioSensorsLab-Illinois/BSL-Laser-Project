...
| `operate.set_target` | stage the runtime TEC temperature or wavelength target | accepted pre-deployment and post-ready-idle (2026-04-20); rejected only while `deployment.running` so mid-checklist TEC_SETTLE is not drifted. Aliases: `set_target_temp`, `set_target_lambda`. |
...
- Runtime control commands `operate.set_output` and `operate.set_modulation` are rejected unless deployment mode is active and the deployment checklist has completed successfully in the current session.
- Target-staging commands `operate.set_target`, `set_target_temp`, `set_target_lambda` (2026-04-20) are separated from the runtime-control group: they accept pre-deployment and post-ready-idle payloads so a host can stage the operator's wavelength before firing `deployment.enter`. They are rejected only while `deployment.running` to protect mid-checklist `TEC_SETTLE` from a target drift.
- `integrate.set_safety` accepts an optional `auto_deploy_on_boot` (boolean). Sets / clears bit 0 of `config.service_flags` (`LASER_CONTROLLER_SERVICE_FLAG_AUTO_DEPLOY_ON_BOOT`). Persisted to a standalone NVS u32 key (`svc_flags` in the `laser_ctrl` namespace) so the v5 / v6 profile migration chain stays untouched. Takes effect at the NEXT power cycle: on boot, if the flag is set, firmware auto-runs `enter_deployment_mode` + `run_deployment_sequence` as soon as a clean-idle state is reached (boot complete, config valid, no fault, no service mode). Emitted back in the safety telemetry block as `autoDeployOnBoot` (bool).
